 ...
 * the initial frontend request. Identical to the message contents when
 * receiving directly from client-updater-service.
 */
 ...
 * @returns     The response data.
 */
export type Handler = (request: object) => Promise<unknown>;
...
export interface AmqpConfig {
  ...
  /**
   * Contents of the `type` field in a success response. `type` is used in the
   * frontend to call explicit callback functions.
   */
  successType: string;
  /**
   * Contents of the `type` field in an error response. `type` is used in the
   * frontend to call explicit callback functions.
   */
  errorType: string;
  /**
...
/**
 * The body to pass to the handlers. contains an optional `action` field for
